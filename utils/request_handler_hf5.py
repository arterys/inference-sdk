...
def create_h5_output(outh5, predictions, preprocess_res):
    ...
    for slice_index, pred in enumerate(predictions):
        ...
        slice_dset = outh5.create_dataset(
            'slice_{}_pred'.format(slice_index), data=pred, dtype=np.float32, compression="gzip"
        )
        ...
        slice_dset.attrs['image_orientation_patient'] = preprocess_res["image_orientation_patient"]

        # Ordered by Arterys slice_index asc
        slice_dset.attrs['centroid'] = preprocess_res["slice_centroids"][slice_index]
        slice_dset.attrs['first_element_center'] = preprocess_res["first_element_centers"][slice_index]
        slice_dset.attrs['centroid_offset'] = preprocess_res["slice_centroid_offsets"][slice_index]
    outh5.close()
...
def request_handler(json_input, dicom_instances, input_digest):
    ...
    transaction_logger.info(
        "server received json_input len={}".format(len(json_input))
    )
